...
export default function Game() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const storedData = JSON.parse(localStorage.getItem('gameConditions'));
        if (storedData) {
          console.log('stored data: ', storedData);
          console.log('GETTING FROM LOCAL STORAGE');
          setGameConditions({
            location: storedData.location,
            windDirection: storedData.windDirection,
          ...
          const gameConditionsObject = {
            location: gameConditions.location,
            windSpeed: gameConditions.windSpeed,
            windDirection: gameConditions.windDirection,
          };

          localStorage.setItem(
            'gameConditions',
            JSON.stringify(gameConditionsObject)
          );
  ...
  return (
    <div className={styles.backgroundContainer}>
      ...
      <GameConditions
        location={gameConditions.location}
        wind={gameConditions.windDirection}
        distance={gameConditions.distance}
      />
      <img src={planes[currentIndex]} alt='orv9' className='orv-9' />
